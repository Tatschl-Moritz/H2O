...
```
GitHub Actions (cron)  →  scrapes / aggregates data  →  commits JSON to public/  →  Coolify redeploys
```
...
- Docker + [Coolify](https://coolify.io) — hosting, redeploys automatically on every push (self-hosted on Hetzner Cloud)
...
The project is a plain Dockerized static site — no platform-specific config required. A multi-stage `Dockerfile` builds the Vite app (`node:20-alpine`) and serves the output with `nginx:alpine`.

```bash
docker compose up --build   # http://localhost:8080
```

In Coolify, point a new resource at this repo; it detects the `Dockerfile` and builds/deploys automatically on every push to `main`. No build/start command overrides needed — the container just serves on port 80.

The two GitHub Actions workflows are what keep the underlying data fresh; every commit they push to `public/` triggers a redeploy automatically, same as before.
